...
async def get_diagram(flow_id :int,token = Depends(auth_handler.auth_wrapper)):
    ...
    try:
        ...
        for conn in all_connections:
            get_conn = {"id": str(conn.id), "markerEnd": {"type": "arrow","color":"#79E794","orient":"auto"},"type": 'buttonedge', "source": str(conn.source_node_id), "sourceHandle": conn.sub_node_id,"target": str(conn.target_node_id), "animated": True, "label": 'edge label', "flow_id":flow_id}
