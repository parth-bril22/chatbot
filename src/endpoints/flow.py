...
async def get_diagram(flow_id :int,token = Depends(auth_handler.auth_wrapper)):
    try:
        ...
        for con in all_connections:
            get_con = {"id": con.id, "markerEnd": {"type": "arrow",},"type": 'buttonedge', "source": con.source_node_id, "sourceHandle": con.sub_node_id,"target": con.target_node_id, "animated": True, "label": 'edge label', "flow_id":flow_id}
            cons.append(get_con)
